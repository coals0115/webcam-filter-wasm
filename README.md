...
```cpp
// sqrt() 대신 제곱값 비교로 성능 최적화
int distSq = dr * dr + dg * dg + db * db;
if (distSq <= tolSq) {
    frame[idx] = bg[idx];  // 배경으로 대체
}
```
...
// 6단계 그라데이션 매핑
if (brightness < 43) { r=0; g=0; b=(brightness*255)/43; }       // 검정→파랑
else if (brightness < 85) { r=0; g=((brightness-43)*255)/42; b=255; }  // 파랑→시안
// ... 녹색→노랑→빨강→흰색
```
...
### 방채민
- **어려움 1**: JS↔WASM 경계 교차 비용 이해
  - 픽셀마다 WASM 함수를 호출하면 프레임당 30만 회 이상 호출이 발생
  - 조사를 통해 경계 교차 비용이 크다는 것을 학습
- **적용**: `HEAPU8.set()`과 `HEAPU8.subarray()`로 데이터를 일괄 복사하는 방식 적용

- **어려움 2**: 가비지 컬렉션(GC) 부담 이해
  - 매 프레임 새 버퍼 할당 시 GC가 빈번하게 발생한다는 것을 학습
- **적용**: 앱 시작 시 버퍼를 한 번만 할당하고 재사용하는 방식 적용
